...
int main(int argc, char **argv)
{
	...
	result = M0_GETOPTS("m0addb2dump", argc, argv,
			...
			M0_FORMATARG('c', "Continuous dump interval (sec.)",
				     "%i", &delay),
			M0_STRINGARG('l', "Motr library path",
...
static void op_state(struct m0_addb2__context *ctx, const uint64_t *v,
                            char *buf)
{
	sm_state(&m0_op_conf, ctx, v, buf);
}

static void state_counter(struct m0_addb2__context *ctx, char *buf)
{
	sm_trans(&m0_op_conf, "op", ctx, buf);
}
                          ...
                          char *buf)
{
	...
	const struct m0_xcode_enum_val *xev =
		&m0_xc_m0_cob_req_states_enum.xe_val[id];
...
static void attr(struct m0_addb2__context *ctx, const uint64_t *v, char *buf)
{
	...
	static const struct range_xen {
		...
		{ M0_AVI_CLIENT_RANGE_START, M0_AVI_DIX_RANGE_START,
		  &m0_xc_m0_avi_labels_enum },
		{ M0_AVI_DIX_RANGE_START, M0_AVI_KEM_RANGE_START,
		  &m0_xc_m0_avi_dix_labels_enum },
		{ M0_AVI_CAS_RANGE_START, M0_AVI_CLIENT_RANGE_START,
	...
	static struct {
		...
		{ &m0_xc_m0_avi_labels_enum, client_values,
		  ARRAY_SIZE(client_values) },
	...
	/*
	 * A very specific case for Client opcode which is presented as
	 * several enums.
	 */
	if (attr_name == M0_AVI_OP_ATTR_CODE) {
		M0_ASSERT(attr_val_xen == NULL);
		for (i = 0; i < ARRAY_SIZE(client_opcode_xc); ++i) {
			if (attr_val >= client_opcode_xc[i].start &&
			    attr_val < client_opcode_xc[i].end) {
				attr_val_xen = client_opcode_xc[i].xen;
				break;
			}
		}
	}
...
struct m0_addb2__id_intrp ids[] = {
	...
	{ M0_AVI_CLIENT_SM_OP_COUNTER, "",
	  .ii_repeat = M0_AVI_CLIENT_SM_OP_COUNTER_END - M0_AVI_CLIENT_SM_OP_COUNTER,
	  .ii_spec   = &state_counter },
	...
	{ M0_AVI_CLIENT_TO_DIX,      "client-to-dix", { &dec, &dec },
	  { "client_id", "dix_id" } },
	...
	{ M0_AVI_IOO_REQ,         "ioo-req-state", { &ioo_req_state, SKIP2 } },
	{ M0_AVI_IOO_REQ_COUNTER, "",
	  .ii_repeat = M0_AVI_IOO_REQ_COUNTER_END - M0_AVI_IOO_REQ_COUNTER,
	...
	{ M0_AVI_CLIENT_BULK_TO_RPC,   "bulk-to-rpc",    { &dec, &dec },
	  { "bulk_id", "rpc_id" } },
